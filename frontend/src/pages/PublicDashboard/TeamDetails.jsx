...
function TeamsdetailPage() {
  ...
  const fetchPlayers = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/teams/teams/${teamId}/players`
      );
      const data = await response.json();
  ...
  const fetchGames = useCallback(async () => {
    ...
    try {
      const response = await fetch(
        `http://127.0.0.1:8000/api/games/game-teams/?team=${teamId}`
      );
      const data = await response.json();
  ...
  useEffect(() => {
    ...
    fetchPlayers();
    fetchGames();
  }, [fetchTeam, fetchPlayers, fetchGames]);
        ...
        {activeSection === "Overview" && (
          <div>
            <div className={styles.overview}>
              ...
              <h3>
                Manager : {teamdetails?.manager?.first_name}{" "}
                {teamdetails?.manager?.last_name}{" "}
              </h3>
            </div>
          </div>
        )}
